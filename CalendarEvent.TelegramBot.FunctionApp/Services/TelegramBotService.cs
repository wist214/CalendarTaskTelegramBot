...
namespace CalendarEvent.TelegramBot.Services
{
    ...
    public class TelegramBotService : ITelegramBotService
    {
        ...
        private async Task HandleAudioMessage(string userId, long chatId, Audio audio, CancellationToken ct)
        {
            ...
            if (audio.Duration > 60) // Limit audio messages to 60 seconds
            {
                _logger.LogWarning("Audio message too long: {Duration}s from user {UserId}", audio.Duration, userId);
                return;
            }

            // Treat audio messages the same as voice messages
            await _mediator.Send(new ProcessVoiceMessageCommand(userId, chatId, audio.FileId, audio.Duration), ct);
        }
    }
}
